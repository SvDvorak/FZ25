...
public partial class SpriteAnimator
{
			...
			case eAnimEventParameter.AudioClip:
				{
					xOffset += width + 5;
					width = 150;
					animEvent.m_paramAudioClipReference = EditorGUI.ObjectField(new Rect(xOffset, 2, width, rect.height - 3), animEvent.m_paramAudioClipReference, typeof(AudioClip), false);
				}
				break;
	...
	{		
		if ( m_acList.Count <= 0 || m_acCanceled ) // TODO: don't show if remaining matches only 
			return; 
		
		float offsetX = 0;//-style.CalcSize( new GUIContent(m_acRemaining) ).x+18;
		// Adjust rect based on size of text
		rect.min = rect.min + new Vector2(offsetX,-15*m_acList.Count);
		// Set x/y based on cursor pos
		rect.max = rect.min + new Vector2( 120 , 15 * m_acList.Count );
		...
		for ( int i = start; i < start+5 && i < m_acList.Count; ++i )
		{
			if ( m_acSelectedIndex == i )
				EditorGUI.DrawRect(new Rect(rect){yMin = rect.yMin+((i-start)*15), height = 15},  GUI.skin.settings.selectionColor );			
			GUI.Label(new Rect(rect){yMin = rect.yMin+((i-start)*15)},m_acList[i].m_functionName, EditorStyles.label);
			//EditorGUI.LabelField(new Rect(rect){yMin = rect.yMin+((i-start)*15)},m_acList[i].m_functionName, EditorStyles.label);
		}
