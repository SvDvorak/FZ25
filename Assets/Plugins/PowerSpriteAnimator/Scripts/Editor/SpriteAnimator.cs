...
public partial class SpriteAnimator : EditorWindow 
{
	...
	class AnimEvent
	{
		...
		public Object m_paramAudioClipReference = null;
	...
	{
		public PreviewRenderUtility m_prevRender;
	...
	static readonly float INFO_PANEL_WIDTH = 260;
	...
	{	
		...
		Rect previewRect = new Rect(lastRect.xMin, lastRect.yMax, position.width-INFO_PANEL_WIDTH, position.height-lastRect.yMax-TIMELINE_HEIGHT);
		...
		// Info Panel
		//
		Rect infoPanelRect = new Rect(lastRect.xMin+position.width-INFO_PANEL_WIDTH, lastRect.yMax, INFO_PANEL_WIDTH, position.height-lastRect.yMax-TIMELINE_HEIGHT);
	...
	void LayoutPreview( Rect rect )
	{
		...
		if ( sprite != null )
		{
			...
			// In 2017.1 Can't display packed sprites at the moment, so don't bother trying
			if ( sprite.packed && Application.isPlaying  )
			{
				EditorGUI.LabelField(rect,"Disabled in Play Mode", new GUIStyle( EditorStyles.boldLabel) { alignment = TextAnchor.MiddleCenter, normal = { textColor = Color.white } } );
				return;
			}
            #endif
	...
	public void LayoutFrameSpriteRendered(Rect rect, Sprite sprite, float scale, Vector2 offset, bool useTextureRect, bool clipToRect, float angle = 0 )
	{
		...
		if ( m_spriteRenderData.TryGetValue( sprite, out data ) == false )
		{
			...
			data.m_prevRender = new PreviewRenderUtility();
				...
				previewCamera = data.m_prevRender.camera;
			#else 
				previewCamera = data.m_prevRender.m_Camera;
			#endif
		...
		// Draw the mesh
		data.m_prevRender.DrawMesh(data.m_previewMesh, pivotOffset / sprite.pixelsPerUnit, Quaternion.Euler(0,0,angle), data.m_mat, 0);
	...
	void OnClipChange( bool resetPreview = true )
	{
		...
		for ( int i = 0; i < events.Length; ++i )
		{
			...
			if (animEvent.objectReferenceParameter as AudioClip)
			{
				newEvent.m_paramAudioClipReference = animEvent.objectReferenceParameter;
				newEvent.m_paramObjectReference = null;
			}	
			...
			if ( newEvent.m_sendUpwards )
			{
				...
				switch (newEvent.m_paramType)
				{
					...
					case eAnimEventParameter.AudioClip:
					{
			...
			else 
			{
				...
				else if (newEvent.m_paramAudioClipReference != null)
					newEvent.m_paramType = eAnimEventParameter.AudioClip;
			}
	...
	void ApplyChanges()
	{
		...
		for ( int i = 0; i < m_events.Count; ++i )
		{
			...
			// Bit of a hack here to speed up the anim event audio pipeline
			if (from.m_paramAudioClipReference != null && functionName == "")
			...
			if ( from.m_sendUpwards )
			{
				...
				switch( from.m_paramType )
				{
					...
					case eAnimEventParameter.AudioClip:
					{	
						// Object/AudioClip events are split into two. The first is the string function name, second is object name					
						...
						if(from.m_paramType == eAnimEventParameter.AudioClip)
							to.objectReferenceParameter = from.m_paramAudioClipReference;
						else
			...
			else 
			{
				...
				if (!to.objectReferenceParameter)
					to.objectReferenceParameter = from.m_paramAudioClipReference;
			}
		}
		...
		foreach ( AnimEvent animEvent in m_events )
		{
			if ( animEvent.m_functionName.Length > 4
